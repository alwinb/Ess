"use strict"
const Map = require ('../lib/aatree.js')
...
// interprtation and a compare function that defines a total order
...
function groupByRank (G, cmp, alg, heap) {
  const Gfn = G (x => ranks[x]) // subnode dereference
	...
	let table = new Map (cmp)     // Map from a rank to an array of nodes
...
function groupByCorank (G, cmpD, decorate, out, d, x) {
  let colors = new Map (compareJs) // Map from elements to their colour
  let table = new Map (cmpD) // Map from colours to arrays of elements
  let columns = 1
...
function layout2 (picFor, grouped, heap) { // Can I get rid of the 'heap' here?
  let rows = []
  let positions = []
  let width = 0
  let height = 0
  ...
  // horizontal added space needed
	...
	grouped.layers.forEach (function (row, row_key) { // row[i]:X
    var renderedRow = new Row ()
    renderedRow.y = height
    for (let item of row) {
      // log ('layers', item, heap[item])
      renderedRow.add ({ id:item, node:heap[item], pic:picFor (heap[item]) })
    }
  ...
  for (let renderedRow of rows) {
    ...
    // now moving bottom up
      ...
      for (let a of pic.arcs) {
        C.arc (a.class, addPt (pt, a.from), a.dir, positions[node[a.for]])
      }
    }
  }
  return C
...
function Canvas (rect_ = unitRect) {
  ...
  this.render = function render () {
    ...
    <svg width="${_round(w)}" height="${_round(h)}">
      <g transform="translate(${_round(-rect.left)} ${(-rect.top)})">
        ${arcs.join('')}
  ...
  this.grow = function (pt) {
  	rect = grow (rect, pt)
  }

  this.path = function path (name, p, d) {
  	nodes.push (`<path transform="translate(${p.x} ${p.y})" class="${name}" d="${d}" />`+'\n') }
