...
function layout2 (layers, picFor, getSub) {
  ...
  // horizontal added space needed
    ...
    for (let item of row) {
      renderedRow.add ({ id:item, node:item, pic:picFor (item) })
    }
  ...
  for (let renderedRow of rows) {
    ...
    // now moving bottom up
      ...
      for (let a of pic.arcs) {
        C.arc (a.class, addPt (pt, a.from), a.dir, positions.get (getSub (node, a.for) ))
      }
    }
  }
  return C
...
function Canvas (rect_ = unitRect) {
  ...
  this.rect = rect // bounding rectangle
